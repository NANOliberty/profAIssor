...
def build_evaluate_prompt(
    ...
    """자료 맥락을 참고해 답변을 평가하고, 답변 불가 시 짧은 보충 힌트를 생성합니다."""
    allow_followup = turn < max_turns and not is_no_answer
    ...
    if is_no_answer:
        followup_rule = (
            ...
            "같은 내용을 다시 묻는 꼬리질문을 만들지 말고 followup과 "
        ...
        result_rule = (
            "답변 불가 전용 필드를 위 규칙대로 작성하고 일반 평가를 수행하지 마세요."
        )
    else:
        if allow_followup:
            followup_rule = (
                "꼬리질문 횟수는 반드시 채워야 하는 목표가 아니라 허용 가능한 최대치입니다. "
                "학생 답변에 질문 문장이 명시한 요구의 누락, 사실 오류 또는 의미상 모호함이 "
                "실제로 있을 때만 꼬리질문을 만드세요. "
                "답변이 짧거나 더 풍부하게 말할 여지가 있다는 이유만으로 꼬리질문을 만들지 마세요. "
                "rubric에 '보통'이나 '부족'이 있다는 이유만으로도 자동 생성하지 마세요. "
                "질문의 핵심 주제와 관련 슬라이드 범위를 유지하고, 직전 질문에서 명시적으로 요구했지만 "
                "아직 답하지 않은 한 부분만 물으세요. "
                "관련된 내용이라는 이유로 로그 종류, 장애 처리 절차, 추가 사례처럼 새 하위 주제로 이동하지 마세요. "
                "직전 질문을 동의어나 유사 표현으로 다시 묻지 마세요. "
                "예를 들어 '두 개념의 차이를 설명해 주세요' 다음에 "
                "'각각의 역할을 더 자세히 설명해 주세요'라고 묻는 것은 중복 질문입니다. "
                f"{followup_difficulty_rule}"
            )
        else:
            followup_rule = (
                "허용 횟수에 도달했으므로 followup과 followup_question_type은 반드시 null로 두세요."
            )

        answer_mode_rule = (
            "answer_status는 answered로 작성하고 supplement는 null, related_slides는 빈 배열로 두세요. "
        ...
        result_rule = (
            "답변이 질문의 명시적 요구를 모두 충족하면 남은 횟수와 관계없이 followup은 null입니다. "
            "이 경우 verdict는 충분, gaps는 없음으로 작성하세요. "
