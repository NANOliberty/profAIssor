...
* [ ] **1-4B / P0-2 — 난이도와 꼬리질문 재조정**

  * 쉬움·보통·어려움을 자료 범위, 추론 단계, 답변 깊이로 구분한다.
  * persona는 질문 관점을 결정하고 난이도는 해당 관점을 파고드는 깊이만 결정하게 한다.
  * 최초 질문과 꼬리질문이 동일 난이도와 동일 쟁점을 유지하도록 한다.
...
* 기본 발표 평가자 선택 상태로 스파링을 시작한다.
* 기본 질문이 발표의 목적·핵심 주장·결과·의의 중 하나를 확인하는지 검토한다.
* 기본 질문이 반례·예외·전문 이론을 먼저 요구하지 않는지 확인한다.
* 교수·동료·일반 청중을 각각 선택했을 때 기존 관점이 유지되는지 확인한다.
* `python -m py_compile backend/personas.py`
* `npm run build`
